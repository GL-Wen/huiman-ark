...
export const buildComicReferenceInstruction = ({
  ...
  if (hasCharacterReference) {
    return '接下来会提供 1 张角色设定图。先锁定角色身份，再完成剧情分镜：人物脸型、发型、服装、主配色与体态必须和该图一致，除非剧情明确要求变化，否则不要改造角色。';
  }
...
export const buildComicReferenceLabels = ({
  ...
  if (hasCharacterReference) {
    labels.push(
      '参考图 1：这是角色设定图，仅用于锁定人物身份。请严格对齐脸型、发型、服装、主配色、体态和年龄感。',
    );
...
10. 如果模型倾向添加角落装饰字样、站点名或伪水印，必须省略这些元素，保持画面干净
...
export const buildComicPagePrompt = ({
  ...
  if (hasCharacterReference) {
    referenceTextArray.push(
      '【角色一致性】已提供角色设定图（首张参考图）。先对齐这张图，再处理剧情：人物五官、发型、服装、配色、年龄感与体态必须保持一致；若文本细节与设定图冲突，以设定图中的外观为准，文本只补充动作、表情、道具和场景。若参考图里含有水印、平台标识、logo、签名、页码或边角文字，一律忽略。',
    );
  } else if (referenceImagesCount) {
    referenceTextArray.push(
...
12. 严禁出现任何平台标识、站点名称、水印、logo、签名、二维码、印章、页脚信息或无关字母数字，例如“U17”这类角落标识
