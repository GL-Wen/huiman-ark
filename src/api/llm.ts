...
export const buildComicReferenceInstruction = ({
  ...
  if (characterReferenceMode === 'portraits') {
    return `接下来会提供 ${referenceImagesCount} 张单人角色立绘参考图，每张图对应一个角色，图片标签标明了角色姓名。这些参考图是角色外观的唯一标准，优先级高于任何文字描述：请逐张识别对应角色，将其脸型轮廓、五官比例、眼型瞳色、发型发色、服装款式与层次、主配色、体态比例完整复刻到漫画中；无论当前分格是远景、中景还是特写，该角色的外观都必须与参考图保持一致，不得自行调整、简化或"优化"；不同角色的参考图之间严禁串色、串脸或混用服装；参考图中如有水印或装饰文字，一律忽略。`;
  }

  if (characterReferenceMode === 'sheet') {
    return '接下来会提供 1 张角色设定总图（Character Design Sheet），图中并排展示多个角色，每个角色旁边标有名称。这张总图是所有角色外观的唯一标准，优先级高于任何文字描述：请先逐一辨认每个角色的独立区域，将其脸型轮廓、五官比例、眼型瞳色、发型发色、服装款式与层次、主配色和体态比例完整记录；在绘制每个分格时，每次出现某个角色都必须回溯到总图中该角色的对应区域进行像素级比对，确保外观完全一致，不得因景别变化、构图需要或风格倾向而自行改造任何视觉特征；参考图中如有水印或装饰文字，一律忽略。';
  }
...
export const buildComicPagePrompt = ({
  ...
  const storyContext = storyInput.length > 200 ? `${storyInput.slice(0, 200)}…` : storyInput;

  const characterNamesText = characterNames.length ? characterNames.join('、') : '请根据段落内容判断';

  return `请生成一张单页多格漫画，准确表现当前这一个段落的剧情。

${referenceText}

━━━ 第一步：锁定角色外观（最高优先级，进入分镜前必须完成） ━━━
本页涉及角色：${characterNamesText}
角色外观定义（以下描述与参考图共同构成角色外观的唯一标准）：
${characterAnchorText}
外观锁定原则：上方每个角色的脸型、五官比例、眼型瞳色、发型发色、服装款式与配色是本次绘制的硬约束。无论分格景别如何变化（远景/中景/特写），无论光线和角度如何调整，每个角色的上述特征必须与参考图和角色描述严格一致，不允许任何形式的”风格化调整”或”美化改造”。
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
...
【动漫风格】${animeStyle}
【风格偏好】${styleText}
【标题限制】”${segment.title}”和”${segment.cut}”仅供理解段落，禁止渲染到图片任何位置。
...
5. 对白和旁白尽量精炼，适合直接放入漫画气泡；每条台词只在唯一一个气泡中出现一次
6. 未在当前段落中明确出现的角色不要随意出镜
7. 不要添加段落标题、章节名、页眉、页码等说明性文字
8. 画面四角和边缘必须保持干净，严禁出现任何水印、平台标识、logo 或无关字母数字
